...
class Account(salesforce.models.Model):
    AccountNumber = fields.CharField(max_length=40)
    AccountSource = fields.CharField(choices=SOURCES)
    IsActive = fields.BooleanField(custom=True)
    AnnualRevenue = fields.FloatField()
    CustomerPriority = fields.CharField(choices=CUSTOMER_PRIORITIES)
    DandbCompanyId = fields.CharField(max_length=18)
    DunsNumber = fields.CharField(max_length=9)
    Fax = fields.CharField(max_length=20)
    Industry = fields.CharField(max_length=40)
    #Owner = models.ForeignKey(User, on_delete=models.PROTECT, db_column='OwnerId')
